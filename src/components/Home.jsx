...
export default function Home() {
    ...
    if (state.running) {
      ...
      return (
        <Button
          ...
          onClick={() => {
            window.electron
              .invoke("runSystem", state.address)
              .then(res => {
  ...
  const makeIfaceOptions = () => {
    ...
    if (keys.length > 0) {
      keys.forEach(key => {
        ...
        v4s.forEach(v4 =>
          out.push({
            label: `${key} --- ${v4.address}`,
            value: { name: key, address: v4.address },
          })
        );
      });
    }
  ...
  const setInterface = data => {
    window.electron
      .invoke("setInterface", data.value)
      .then(res => setcurrentInterface(res))
      .catch(err => console.error(err));
  };
  ...
  const makeIfaceValue = () => {
    if (currentInterface === null) return null;
      ...
      return {
        label: `${currentInterface.name} --- ${currentInterface.address}`,
        value: {
  ...
  const renderTooltip = (
    <Tooltip>
      ...
      <div>
        Rendering universes is taxing on the system and should only be done
  ...
  return (
    <div>
      ...
      >
        ...
        />
        <div style={{ padding: "0px 8px" }}>{makeRunStop()}</div>
      </div>
      <div style={{ marginTop: "10px", display: "flex", alignItems: "center" }}>
        {makeUniversesButton()}
        <div style={{ padding: "0px 10px", fontSize: "12px" }}>
          Set artnet destination address to interface address. Enable output to
          start sending artnet to venue ip.
        </div>
      </div>
      <hr />
      {makeUniverses()}
